...
export default function AboutSection() {
  ...
  return (
    <section id="about" ref={ref} style={{
      background: 'var(--bg-mid)', padding: '160px 80px',
      ...
      <motion.div style={{
        ...
        y: yParallax,
        fontFamily: 'var(--font-display)', fontSize: 'clamp(140px, 25vw, 400px)',
      ...
      >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '80px', alignItems: 'start', marginBottom: '80px' }}>
            <motion.h2 variants={fadeUp} style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'clamp(36px, 4.5vw, 68px)', lineHeight: 1.05, letterSpacing: '-0.035em', color: 'var(--text-light)' }}>
              {lang === 'ar' ? (<>تحويل الأفكار إلى<br /><span style={{ color: 'var(--bg-mid)', textShadow: '-1px -1px 0 var(--text-light), 1px -1px 0 var(--text-light), -1px 1px 0 var(--text-light), 1px 1px 0 var(--text-light)' }}>تجارب</span><br />تترك أثراً.</>)
          ...
          <motion.div variants={fadeUp} style={{ paddingTop: '6px' }}>
              {lang === 'ar' ? (
                <>
                  <p style={{ fontFamily: 'var(--font-body)', fontSize: '17px', lineHeight: 1.85, color: 'var(--text-muted)', marginBottom: '24px' }}>أنا <strong style={{ color: 'var(--text-light)', fontWeight: 600 }}>عابد سيف</strong>، مصمم UI/UX معتمد من <strong style={{ color: 'var(--accent)', fontWeight: 600 }}>معهد كاليفورنيا للفنون، الولايات المتحدة</strong>. بخبرة تتجاوز 8 سنوات في تصميم واجهات مستخدم بديهية تقدم تجارب سلسة.</p>
                  <p style={{ fontFamily: 'var(--font-body)', fontSize: '17px', lineHeight: 1.85, color: 'var(--text-muted)', marginBottom: '36px' }}>من تطبيقات الجوال إلى استراتيجيات العلامة التجارية الكاملة، أبقى على اطلاع بأحدث اتجاهات التصميم — ومقيم حالياً في <strong style={{ color: 'var(--text-light)', fontWeight: 500 }}>الدوحة، قطر</strong>.</p>
                </>
              ) : (
                <>
                  <p style={{ fontFamily: 'var(--font-body)', fontSize: '17px', lineHeight: 1.85, color: 'var(--text-muted)', marginBottom: '24px' }}>I'm <strong style={{ color: 'var(--text-light)', fontWeight: 600 }}>Abid Saif</strong>, a certified UI/UX designer with a specialization from the <strong style={{ color: 'var(--accent)', fontWeight: 600 }}>California Institute of Arts, USA</strong>. With 8+ years of experience, I create intuitive interfaces that deliver seamless user experiences.</p>
                  <p style={{ fontFamily: 'var(--font-body)', fontSize: '17px', lineHeight: 1.85, color: 'var(--text-muted)', marginBottom: '36px' }}>From mobile apps to full brand strategies, I stay updated with the latest design trends — currently based in <strong style={{ color: 'var(--text-light)', fontWeight: 500 }}>Doha, Qatar</strong>.</p>
                </>
              )}
            ...
            <div style={{
              ...
              border: '1px solid var(--border)', background: 'rgba(201,169,110,0.06)',
              marginBottom: '28px',
            }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
              {disciplines.map(d => (
                <span key={d} className="chip" style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: '11px' }}>{d}</span>
              ))}
            </div>
          </motion.div>
        </div>
        ...
        <motion.div variants={fadeUp} style={{
          ...
          {stats.map(({ value, label }, i) => (
            <div key={label} style={{
              ...
              <div style={{
                fontFamily: 'var(--font-display)', fontSize: 'clamp(44px, 5.5vw, 76px)',
            ...
            {tools.map(({ name, icon }) => (
              ...
              >
                ...
                <span style={{
                  ...
                  color: 'var(--text-light)', letterSpacing: '-0.01em',
                }}>{name}</span>
              </motion.div>
            ))}
          </div>
            ...
            {certs.map((c, i) => (
              ...
              >
                <div style={{
                  ...
                  letterSpacing: '0.15em', color: 'var(--accent)', marginBottom: '10px',
                }}>{c.year}</div>
                <div style={{
                  fontFamily: 'var(--font-display)', fontSize: '14px', fontWeight: 700,
                  color: 'var(--text-light)', marginBottom: '6px', lineHeight: 1.3,
                }}>{c.title}</div>
                <div style={{
