...
function TimelineCard({ job, index, jobsCount }) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Card header */}
        <div style={{ padding: '28px 32px', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '20px' }}>
          <div style={{ flex: 1 }}>
            ...
            <h3 style={{
              ...
              fontSize: 'clamp(18px, 2.5vw, 26px)', lineHeight: 1.1,
              color: 'var(--text-light)', letterSpacing: '-0.02em', marginBottom: '6px',
            }}>{job.role}</h3>
            <p style={{
              fontFamily: 'var(--font-body)', fontSize: '14px',
              color: 'var(--text-muted)',
            }}>{job.company} · <span style={{ color: 'rgba(var(--text-rgb),0.4)' }}>{job.sub}</span></p>
          </div>
          <motion.div
  ...
  return (
    <section id="experience" ref={ref} style={{
      ...
      <motion.div style={{
        ...
        y: yParallax,
        fontFamily: 'var(--font-display)', fontSize: 'clamp(120px, 20vw, 320px)',
      ...
      <div style={{ maxWidth: '1000px', margin: '0 auto', position: 'relative', zIndex: 1 }}>
        ...
        >
          ...
          <h2 style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'clamp(42px, 6vw, 84px)', lineHeight: 1.0, letterSpacing: '-0.04em', color: 'var(--text-light)' }}>
            {lang === 'ar' ? (<>٨ سنوات من<br /><span style={{ color: 'var(--accent)' }}>قيادة التصميم.</span></>) : (<>8 years of<br /><span style={{ color: 'var(--accent)' }}>leading design.</span></>)}
          </h2>
        </motion.div>
